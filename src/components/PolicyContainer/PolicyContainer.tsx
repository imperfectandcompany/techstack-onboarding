...
function PolicyContainer({ setShowPolicy, showPolicy, Sections, Title, Subtitle, loggedIn = false }: any) {
    ...
    return loggedIn ? (
        <IonModal isOpen={showPolicy} swipeToClose={true} canDismiss={true} color='secondary' onDidDismiss={() => setShowPolicy(false)} >
            <IonHeader collapse="condense">
                <IonToolbar className={`${styles['modal-toolbar']}`}>
                    <IonButtons slot="start" className="cursor-pointer focus:cursor-default" onClick={() => setShowPolicy(false)}>
                        <IonButton expand="block" size="large"
                            onClick={() => setShowPolicy(false)}
                            className="transition cursor-pointer select-none focus:select-none hover:text-gray-100 focus:text-opacity-50" color='primary' fill="clear">
                            <IonIcon slot="icon-only" color='secondary' icon={chevronDown} />
                        </IonButton>
                    </IonButtons>
        ...
        <IonModal isOpen={showPolicy} swipeToClose={true} canDismiss={true} className={`${styles.modal}`} onDidDismiss={() => setShowPolicy(false)} >
            <IonHeader collapse="condense">
                <IonToolbar className={`${styles['modal-toolbar']}`}>
                    <IonButtons slot="start" className="cursor-pointer focus:cursor-default" onClick={() => setShowPolicy(false)}>
                        <IonButton expand="block" size="large"
