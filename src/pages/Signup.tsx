import { CreateAnimation, IonBackButton, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonInput, IonItem, IonLabel, IonList, IonListHeader, IonNote, IonPage, IonRouterLink, IonTitle, IonToolbar } from '@ionic/react';
...
const Signup: React.FC = () => {
  ...
  const nextPage = async (enteringElement: any, leavingElement: any) => {
    ...
    await pageTransition(signUpRef.current, {enteringEl: enteringElement.current, leavingEl: leavingElement.current, direction: 'forward'}).play()
    ...
    return (
      ...
      <IonList className={`${styles.transparentbg}`}>
          ...
          <IonItem className="items-end">
            ...
            <IonInput onIonFocus={()=>setConfirmPasswordFocused(true)} onIonBlur={()=>setConfirmPasswordFocused(false)} id="confirmPasswordEnter" class="passwordEnter" autocomplete="current-password" clearOnEdit={true} inputmode="text" enterkeyhint="go" type={showConfirmPassword ? 'text': 'password'} value={confirmPassword} onIonChange={e => setConfirmPassword(e.detail.value!)} required
            ></IonInput>
            {confirmPasswordfocused ? 
            <IonIcon slot="end" className="cursor-pointer select-none animate__animated animate__fadeIn focus:select-none hover:text-gray-200 focus:text-gray-600 focus:text-opacity-50" onClick={()=>toggleConfirmPassword()} icon={showPassword ? eyeOff : eye}/>
            : null}
          </IonItem>
  ...
  const photoUpload = () => {
    ...
    return (
      <IonContent fullscreen ref={photoUploadRef} style={{display: 'none'}}>
              <p className="ion-text-center text-white font-bold m-8 pt-10 text-5xl">
              Welcome!
              </p>
              <p className="ion-text-center text-white font-medium m-6 mt-5 text-md">
              We need a little information from you to help other people get to know you.
              Let's start by picking a profile picture.
              </p> 
              <img src={process.env.PUBLIC_URL + '/assets/icon/PhotoUpload.png'} className="w-64 sm:w-24 md:w-32 lg:w-36 my-8 flex mx-auto" alt="logo" />
            <div className="ion-margin-horizontal">
                              
            </div>
            <IonButton className="flex ion-margin-top ion-margin-horizontal t-8 font-bold" color="dark" fill="outline">
  ...
  const userNameComponent = () => {
    ...
    return (
      <IonContent ref={userNameRef} style={{display: 'none'}}>
            ...
            <IonIcon slot="start" className="cursor-pointer select-none animate__animated animate__fadeIn focus:select-none hover:text-gray-200 focus:text-gray-600 focus:text-opacity-50" icon={at}/>
            <IonInput id="userNameEnter" class="passwordEnter" clearOnEdit={true} inputmode="text" enterkeyhint="go" type='text' value={userName} onIonChange={e => setUserName(e.detail.value!)} required
            ></IonInput>
            
          </IonItem>
          <p className="ion-text-end font-small m-6 text-s" >
                Taken
          </p>
           <div className="flex justify-center mt-6">
            <IonButton expand="block" size="large"
  ...
  const numberEntry = () => {
    ...
    return (
      <IonContent ref={numberRef} style={{display: 'none'}}>
            ...
            <IonInput id="numberEntry" class="passwordEnter" clearOnEdit={true} inputmode="text" enterkeyhint="go" type='number' value={number} onIonChange={e => setNumber(e.detail.value!)} required
            ></IonInput>
          ...
          <div className="ion-margin-horizontal">
              ...
              When you tap Continue, We will send a text with a
              verification code. Message and data rates may apply.
              The verified phone number can be used to login. <a className="underline">Learn what happens when your number changes.</a>
              </p>              
            </div>
           <div className="flex justify-center mt-6">
